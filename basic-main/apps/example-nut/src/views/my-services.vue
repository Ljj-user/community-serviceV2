...
watch([joinedStatusFilter, joinedSortFilter], () => {
  loadData('joined')
})
</script>
    ...
    <ThreeSectionPage page-class="task-page m-mobile-page-bg" content-class="task-content">
      ...
      <section class="page-hero">
        <div>
          ...
          <h2>盯服务进度</h2>
          <span>认领后每一步都能回看</span>
        </div>
        <button type="button" class="hero-action" @click="onTakeTask">
          再接一单
        </button>
      </section>
      ...
      <TaskFeedList
        ...
        :rows="joinedRows"
        :loading="loading"
        :requester-name-map="joinedRequesterNameMap"
        @open="(row) => openClaimDetail(row as any)"
      />
...
.page-hero {
  ...
  gap: 12px;
  background:
    linear-gradient(135deg, rgba(255, 255, 255, 0.9), rgba(240, 249, 244, 0.82)),
    radial-gradient(circle at top right, rgba(21, 128, 61, 0.08), transparent 38%);
