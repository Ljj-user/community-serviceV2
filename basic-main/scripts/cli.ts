...
async function runMode(config: ModeConfig): Promise<void> {
  ...
  // 毕设移动端：仓库内同时存在多个模板 app 时，默认直接启动 example-nut，避免误开模板集合
  if (
    config.action === 'dev'
    && apps.length > 1
    && !hasInteractiveDevArg(process.argv.slice(2))
  ) {
    const pinned = apps.find(
      a => a.name === 'example-nut' || a.packageName === '@fantastic-mobile/example-nut',
    )
    if (pinned) {
      p.log.info('默认启动 example-nut（社区项目移动端）。若要选其它模板应用，请使用: pnpm dev -- -i')
      await runSelectedApps([pinned], config, true)
      return
    }
  }
...
/** 为 true 时保留「多应用选一」的交互菜单（默认不选，见下方毕设默认应用） */
function hasInteractiveDevArg(argv: string[]): boolean {
...
function printHelp(): void {
  ...
  console.log('  pnpm dev -- -i   # 多应用时手动选择（模板 example / example-varlet 等）')
}
