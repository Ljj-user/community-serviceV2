...
async function changePassword() {
  if (!userInfo.value.oldPassword || !userInfo.value.newPassword || !userInfo.value.confirmPassword)
    return

  if (userInfo.value.newPassword !== userInfo.value.confirmPassword) {
    useNotifyStore().error(t('profile.passwordNotMatch'))
    return
  }

  isLoading.value = true
  try {
    const ok = await AccountService.changePassword(
      userInfo.value.oldPassword,
      userInfo.value.newPassword,
    )
    if (ok) {
      useNotifyStore().success(t('profile.changePasswordSuccess'))
      userInfo.value.oldPassword = ''
      userInfo.value.newPassword = ''
      ...
      useNotifyStore().error(t('profile.changePasswordFailed'))
    }
  } catch (e) {
    useNotifyStore().error(t('profile.changePasswordFailed'))
  } finally {
